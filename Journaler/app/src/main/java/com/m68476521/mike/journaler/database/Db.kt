...
object Db : Crud<DbModel> {
    ...
    override fun insert(what: Collection<DbModel>): Boolean {
        ...
        what.forEach { item ->
            when (item) {
                ...
                else -> throw IllegalArgumentException("Unsupported model: $item")
            }
        }
    ...
    override fun delete(what: Collection<DbModel>): Boolean {
        ...
        when (item) {
            is Entry -> {
                when (item) {
                    ...
                    else -> throw IllegalArgumentException("Unsopported entry type: $item")
                }
            }
            else -> throw IllegalArgumentException("Unsopported db model: $item")
        }
        ...
        } else Log.w(tag, "Delete [ Failed ] [ $statement ]")
        db.endTransaction()
        db.close()
        return success
    }
